...
async def get_transitions(
    ...
    financial_year: str = Query(
        "FY24-25", description="Financial year (for customers only)"
    ),
):
    """
    ...
    if analysis_type == "Products":
        # Parse FY to determine which rolling tables to use
        fy_parts = financial_year.replace("FY", "").split("-")
        if len(fy_parts) == 2:
            end_year = int(f"20{fy_parts[1]}")
            # Last 3 months of FY: Jan, Feb, Mar of end_year
            fy_table_suffixes = [f"{end_year}_{m:02d}" for m in [1, 2, 3]]
            table_query = (
                """
                SELECT tablename
                ...
                + " OR ".join(
                    [
                        f"tablename = 'rolling_abc_xyz_summary_{s}'"
                        for s in fy_table_suffixes
                    ]
                )
                + """)
                ORDER BY tablename
        ...
        if not table_rows or len(table_rows) < 1:
            # Fallback: use latest available tables
            fallback_query = """
                SELECT tablename
                FROM pg_catalog.pg_tables
                WHERE schemaname = 'public'
                ...
                LIMIT 3
            """
            table_rows = query_all(fallback_query)  # type: ignore
            if not table_rows or len(table_rows) < 1:
                raise HTTPException(
                    status_code=404,
        ...
        all_products = set()
        month_data = {}
        month_col_order = []  # Maintain chronological order
        ...
        for i, table_name in enumerate(table_names):
            ...
            month_col_order.append(abc_xyz_col_name)
        ...
        product_names_map = {
            row["product_code"]: row.get("product_names", [])
            for row in product_names_rows
        }

        data = []
        for product_id in sorted(
            all_products, key=lambda x: int(x) if x.isdigit() else 0
        ):
            product_names_list = product_names_map.get(product_id, [])
            product_names_str = (
                ", ".join(product_names_list) if product_names_list else "-"
            )
        ...
        column_headers = ["ProductID", "Product Names"] + month_col_order
        ...
        for month_date in last_3_months:
            ...
            month_time_id = (
                (month_date.year - BASE_DATE.year) * 12
                + (month_date.month - BASE_DATE.month)
                + 1
            )
            ...
            for row in rows:
                ...
                cumulative += revenue
                cumulative_pct = (
                    (cumulative / total_revenue * 100) if total_revenue > 0 else 0
                )
        ...
        customer_names_map = {
            row["customer_code"]: row.get("customer", "-")
            for row in customer_names_rows
        }

        data = []
        for customer_id in sorted(
            all_customers, key=lambda x: int(x) if x.isdigit() else 0
        ):
            customer_name = customer_names_map.get(customer_id, "-")

            row_data = {
                "CustomerID": (
                    int(customer_id) if customer_id.isdigit() else customer_id
                ),
        ...
        column_headers = ["CustomerID", "Customer Name"] + month_col_order
